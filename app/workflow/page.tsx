...
//   const filteredTasks = useMemo(() => {
//     return tasks.filter((task: TaskItem) => {
//       if (activeFilter !== "COMPLETED" && isPastDate(task.dueAt)) {
//         return false;
//       }
...
//     if (!response.ok || !data.success) {
//       throw new Error(data.error || "Failed to add task to Calendar.");
//     }
...
//     setPageSuccess("Task added to Calendar successfully.");
//     await loadTasks();

//     window.dispatchEvent(new Event("pulse:auto-sync-complete"));
//   } catch (error) {
//     setPageError(
//       error instanceof Error ? error.message : "Failed to add task to Calendar.",
//     );
//   } finally {
...
//                     </h3>
//                     <p className="mt-1 max-w-md text-sm leading-6 text-slate-500">
//                       Only today and upcoming tasks appear here. Completed tasks
//                       are available in the Completed filter.
//                     </p>
//                   </div>
//                 ) : (
//                   <div className="divide-y divide-slate-100">
...
//   task={task}
//   onClick={() => setSelectedTaskId(task.id)}
//   onMarkDone={() => updateTaskStatus(task.id, "DONE")}
//   onAddToCalendar={() => {
...
//     setCalendarStartTime(start.toISOString().slice(0, 16));
//     setCalendarEndTime(end.toISOString().slice(0, 16));
//   }}
//   onAskAi={() => {
//     setSelectedTaskId(task.id);
//     triggerPulseAi(
...
//         }
//       }}
//       className="grid w-full cursor-pointer grid-cols-[42px_minmax(0,1fr)_120px_120px_130px_44px] items-center gap-4 px-5 py-4 text-left transition hover:bg-emerald-50/50"
//     >
//       <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-slate-100 text-slate-600">
//         <Icon className="h-4 w-4" />
//       </div>
...
//           {isOverdue(task.dueAt) && task.status !== "DONE" && !isPastDate(task.dueAt) && (
//             <span className="rounded-full bg-rose-50 px-2 py-0.5 text-[11px] font-semibold text-rose-700 ring-1 ring-rose-200">
//               Overdue
//             </span>
//           )}
...
//         title={task.status === "DONE" ? "Completed" : "Mark completed"}
...
export default function TasksPage() {
  ...
  const calendarTask = useMemo(
    () => tasks.find((task: TaskItem) => task.id === calendarTaskId) || null,
    [tasks, calendarTaskId],
  );
  ...
  function openCalendarModal(task: TaskItem) {
    setCalendarTaskId(task.id);
    setPageError(null);
    setPageSuccess(null);
    ...
    setCalendarStartTime(toDateTimeLocalValue(safeStart));
    setCalendarEndTime(toDateTimeLocalValue(end));
  }
        ...
        <section className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
          {!selectedTask ? (
                  ...
                  <div className="divide-y divide-slate-100">
                    {filteredTasks.map((task: TaskItem) => (
                      <TaskRow
                        ...
                        onClick={() => setSelectedTaskId(task.id)}
                        onMarkDone={() => updateTaskStatus(task.id, "DONE")}
                        onAddToCalendar={() => openCalendarModal(task)}
                        onAskAi={() => {
                          setSelectedTaskId(task.id);
                          triggerPulseAi(
                            "Explain this task properly. What do I need to do, what is the deadline, and what should be my next step?",
            ...
            <SelectedTaskWorkspace
              ...
              onAddToCalendar={() => openCalendarModal(selectedTask)}
              onMarkProgress={() =>
                updateTaskStatus(selectedTask.id, "IN_PROGRESS")
              }
              onMarkDone={() => updateTaskStatus(selectedTask.id, "DONE")}
            ...
            <div className="mb-5 flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 text-sm leading-6 text-slate-500">
                  Select when this task should appear on your Calendar page.
                </p>
              </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="mt-2 h-11 w-full rounded-xl border border-slate-200 bg-white px-4 text-sm text-slate-900 outline-none focus:border-emerald-300"
                />
              </div>
...
function TaskRow({
  ...
  return (
    <div
      ...
      className="grid w-full cursor-pointer grid-cols-[42px_minmax(0,1fr)_120px_120px_130px_44px_44px] items-center gap-4 px-5 py-4 text-left transition hover:bg-emerald-50/50"
    >
      <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-slate-100 text-slate-600">
        <Icon className="h-4 w-4" />
      </div>
      ...
      <button
        ...
        className="flex h-9 w-9 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-500 transition hover:border-emerald-200 hover:bg-emerald-50 hover:text-emerald-700"
      >
        <CalendarPlus className="h-4 w-4" />
      </button>
